...
Set-AzureRmVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name 'MyFrontEndSubnet'-AddressPrefix '10.3.1.0/24' -NetworkSecurityGroup $nsgfe

  # Create an NSG rule to allow SQL traffic from the front-end subnet to the back-end subnet.
$rule11 = New-AzureRmNetworkSecurityRuleConfig -Name 'Allow-SQL-FrontEnd' -Description "Allow SQL"-Access Allow -Protocol Tcp -Direction Inbound -Priority 110 -SourceAddressPrefix '10.3.1.0/24' -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 1433

#Create an NSG rule to Deny HTTP traffic the Internet to the back-end subnet.
$rule112 = New-AzureRmNetworkSecurityRuleConfig -Name 'Deny-HTTP-BackEnd' -Description "Deny HTTP"-Access Deny -Protocol Tcp -Direction Outbound -Priority 120 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 80


  # Create a network security group for back-end subnet.
$nsgbe = New-AzureRmNetworkSecurityGroup -ResourceGroupName $ResourceGroupName -Location $Location -Name "MyNsg-BackEnd" -SecurityRules $rule1,$rule11,$rule112


  # Associate the back-end NSG to the back-end subnet
Set-AzureRmVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name 'MyBackEndSubnet'-AddressPrefix '10.3.2.0/24' -NetworkSecurityGroup $nsgbe

#Set-AzureNetworkSecurityGroupToSubnet -Name 'MyBackEndSubnet' -VirtualNetworkName $vnet -SubnetName $besubnet
